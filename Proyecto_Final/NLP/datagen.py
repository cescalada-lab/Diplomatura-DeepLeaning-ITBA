...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        if self.data_augmentation:
            smile = SmilesEnumerator()
            X = np.array([smiles_to_seq(smile.randomize_smiles(idx), seq_length = self.seq_length) for idx in self.X[indexes]])
        else:
            X = np.array([smiles_to_seq(idx, seq_length = self.seq_length) for idx in self.X[indexes]])
        y = self.y[indexes]
        return X, y
